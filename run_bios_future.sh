...
# https://opus.nci.org.au/display/Help/How+to+submit+a+job
 #PBS -N ACCESS-ESM1.5-S1-1000pts_test
 #PBS -P x45
 ...
 #PBS -j oe
 #PBS -S /bin/bash
 #PBS -M alison.bennett@csiro.au
 #PBS -m ae

# script varied from original run_cable-pop.sh script
# modified for BIOS3 future runs - most of systems/other users stuff stripped out
...
#
# This script uses CCAM downscaled GCM forcing.
#
...
#          a) using AGCD forcing from 1951 to GCMstart-1, everything dynamic
#          b) CCAM-derived historical data from GCMstart to GCMswitch, everything dynamic
#   7. Future run, everything dynamic from GCMswitch to GCMend using CCAM-derived forcing
#
...
#           Alison Bennett, 2024     - modified to include switches for restart files, spin_data and landmask type.
...
experiment=ACCESS-ESM1.5-S1-1000pts_test   #experiment name
...
# Step 5b
doinidyn=0      # 1/0: Do/Do not full dynamic spinup from 1700 to 1950 (5b)
# Step 6
dofinal1=0    # 1/0: Do/Do not final run from YearStart (default=1951) to GCMstart - use for AGCD
dofinal2=0      # 1/0: Do/Do not final run from GCMstart to GCMswitch - use for historical section of CCAM-derived met
# Step 7
dofuture=1     # 1/0: Do/Do not future runs GCMswitch to GCMend use for future section of CCAM-derived met and era5 (1979-2020)

# --------------------------------------------------------------------
# Other switches
restarttype='None' # 'None' 'AGCD_1951' 'GCM_1951' 'AGCD_1979' 'GCM_2015' 
spin_data='AGCD'     # 'AGCD' 'GCM' # if GCM, updates global nml with historical GCM data after doclimate section. 
landmasktype='land' # 'mask' 'land' # determines the GlobalLandMask. Must match output%grid: mask = gridded, land = points.

# MetType
mettype='bios'        # 'bios' only
domain='aust_0.25_pts'     #  bios domain - 'acttest9','reccap1000pts','aust_0.25_pts','australia'
GCM='ACCESS-ESM1-5'     # 'NorESM2-MM' 'ECMWF-ERA5' 'CNRM-ESM2-1' 'CMCC-ESM2' 'ACCESS-ESM1-5' 'EC-Earth3' 'ACCESS-CM2' 'CESM2'
RCP='ssp370'          # 'historical', 'ssp126', 'ssp370', 'evaluation' - note no future run if historical
GCMstart=1951         #  start year for CCAM derived meteorology - must be between 1951-2014 (GCM), 
GCMsw=2015            #  start year of 'future' in CCAM derived meteorology (2015 for CCAM met, 1979 for ERA5)
GCMend=2099           #  end year for CCAM derived meteorology - must be less than 2100 (2099 for CCAM met, 2020 for ERA5)
...
call_blaze=0        # 1/0 Do/Do not use BLAZE fire model.
...
if [[ "${system}" == "inh599@gadi" || "${system}" == "harman@gadi" ]] ; then
    ...
    cablehome="/home/563/ab7412/cable_code/Github/BLAZE_9184"
else
...
# Met is assumed to be BIOS
  if [[ "${mettype}" == "bios" ]] ; then
   	GlobalMetPath="/g/data/x45/BIOS3_forcing/${domain}/met/"                # last slash is needed | ACB: metpath for global .nml
...
## update namelist with GCM data if using GCM data for spinup

if [[ "${spin_data}" == "GCM" ]] ; then
    MetPath="/g/data/x45/BIOS3_forcing/CCAM/${GCM}/historical/${domain}/met/"
    YearStart=${GCMstart}
	value=$(expr $GCMsw - 1)
    YearEnd=${value}
    echo "using GCM data for spinup"
    echo "met path is " ${MetPath}
...
EOF
    applysed ${tmp}/sedtmp.${pid} ${rdir}/bios_${experiment}.nml # update this file with new met
fi
...
if [[ ${dofinal2} -eq 1 ]] ; then
    ...
    if [[ "${restarttype}" != "None" ]]; then
            if [[ "${restarttype}" == "GCM_1951" ]] ; then
                restart_path="/g/data/x45/BIOS3_forcing/CCAM/restart_files/${restarttype}/${GCM}/${domain}/"
            elif [[ "${restarttype}" == "AGCD_1951" ]]; then
                restart_path="/g/data/x45/BIOS3_forcing/CCAM/restart_files/${restarttype}/${domain}/"
            fi
...
if [[ (${dofuture} -eq 1) && ("${RCP}" != "historical") ]] ; then
    ...
    if [[ "${restarttype}" == "GCM_2015" ]] ; then
        restart_path="/g/data/x45/BIOS3_forcing/CCAM/restart_files/${restarttype}/${GCM}/${domain}/"
        cp ${restart_path}*.nc ${rdir}/restart
    elif [[ "${restarttype}" == "AGCD_1979" ]] ; then
        restart_path="/g/data/x45/BIOS3_forcing/CCAM/restart_files/${restarttype}/${domain}/"
        co2_file="1979_2021_CO2_time_series_ccam_era5.bin"
        cp ${restart_path}*.nc ${rdir}/restart
    fi
